...
export default {
  ...
  methods: {
      ...
      this.$axios.get("/document/content",{
        ...
        if(response.status === 200){
          if(response.data.code === 0){
            ...
            let time =  response.data.modifyTime;
            time = time.split('+')[0];
            time = time.split('T')[0] + ' ' + time.split('T')[1].slice(0,-7);
            this.myModifyTime = time;
      ...
      this.$axios.get("document/edit",{
        ...
        if(response.status === 200){
          if(response.data.code === 0){
            ...
            this.otherEditing = false;
          }else if(response.data.code === 1){
            ElMessage({ message: "其他人正在编辑", type: 'warning'});
            this.otherEditing = true;
    ...
    quitEdit(func){
      ...
      this.$axios.get("document/quit-edit",{
        params : {
          docId: this.myDocId,
          userId : this.myEditor,
        }
        ...
        if(response.status === 200){
          if(response.data.code === 0){
            ...
            ElMessage({ message: "退出编辑失败", type: 'warning'});
          }
        }else{
          ElMessage({ message: "退出编辑失败", type: 'warning'});
        }
    ...
    authorityChanged() {
      console.log('权限或者编辑状态发生变化:' ,this.myAuthority);
      ...
      if(this.myAuthority < 4 || this.otherEditing){
        console.log("文档只读！");
    ...
    quit() {
      this.quitEdit();
      this.$router.push({name:'table',params:{info: this.$store.state.tableInfo}});
    },
  },
  watch: {
    myAuthority(val) {
